...
class MammotionWebRTCCamera(MammotionCameraBaseEntity):
    ...
    async def async_added_to_hass(self) -> None:
        """Sync enabled state with cloud_enabled on startup and every coordinator update."""
        await super().async_added_to_hass()
        self._sync_enabled_state()
        self.async_on_remove(
            self.coordinator.async_add_listener(self._sync_enabled_state)
        )

    @callback
    def _sync_enabled_state(self) -> None:
        """Enable or disable this entity in the registry to match cloud_enabled."""
        registry = er.async_get(self.hass)
        entry = registry.async_get(self.entity_id)
        if entry is None:
            return
        cloud_on = self.coordinator.cloud_enabled
        if cloud_on and entry.disabled_by == er.RegistryEntryDisabler.INTEGRATION:
            registry.async_update_entity(self.entity_id, disabled_by=None)
        elif not cloud_on and entry.disabled_by is None:
